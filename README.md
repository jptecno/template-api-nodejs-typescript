...
Não faça push direto em `development` ou `main`. O ruleset ativo `Proteção de branches permanentes` (ID `20485225`) exige pull request com uma aprovação e resolução das conversas, além de proibir exclusão e force-push nas duas branches. Nenhum status check é obrigatório enquanto o nome do check da CI não for confirmado por uma execução real. Segredos e configurações de testes/produção devem ser configurados no provedor de deploy por ambiente, nunca em branches ou arquivos versionados.
